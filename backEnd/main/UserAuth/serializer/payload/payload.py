...
class MyTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def get_token(cls, user):
        ...
        try:
            ...
            token['phoneNumber'] = str(profile.phoneNumber) if profile.phoneNumber else None
            ...
            logger.error(f"AttributeError for user {user.id}: {str(e)}")
            token['fullName'] = None
            token['NIC_number'] = None
            token['phoneNumber'] = None
            token['country_id'] = None
            ...
            logger.error(f"Unexpected error in token creation for user {user.id}: {str(e)}")
